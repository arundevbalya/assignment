...
import com.assessment.respository.RoleRepository;
...
public class DefaultUserServiceImpl implements DefaultUserService{
	...
	public User save(UserDTO userRegisteredDTO) {
		Role role = new Role();
		if(userRegisteredDTO.getRole().equals("PUBLISHER"))
		  role = roleRepo.findByRole( Constants.ROLE_PUBLISHER);
		else if(userRegisteredDTO.getRole().equals("REVIEWER"))
		 role = roleRepo.findByRole(Constants.ROLE_REVIEWER);
		else if(userRegisteredDTO.getRole().equals("APPROVER"))
			 role = roleRepo.findByRole(Constants.ROLE_APPROVER);
		User user = new User();
